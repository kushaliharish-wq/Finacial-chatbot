...
dimension = 1920  # Ensure this matches the model dimension

# Create or delete existing index
...
def generate_embedding(text):
    ...
    vector = embedding_model.encode(text)  # Using the transformer model
    return vector
...
# Modify the sync_with_pinecone function
def sync_with_pinecone(data, index):
    ...
    for i in tqdm(
        ...
        print(f"Concatenated {len(embeds)} embeddings")

        # Convert embeddings to float64 (double precision)
